...
export async function swapAndAddLiquidity(
  ...
  if (tokenInApproval !== "sent" || tokenOutApproval !== "sent") {
    return "failed";
  }
  ...
  const swapAndAddOptions: any = (address: string) => {
    return {
      swapOptions: {
  ...
  const routeToRatioResponse = async (address: string) => {
    return await router.routeToRatio(
      token0CurrencyAmount,
      token1CurrencyAmount,
      currentPosition,
      swapAndAddConfig,
      swapAndAddOptions(address)
    );
  };
